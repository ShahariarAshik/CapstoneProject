  ...
  const handleSubmit = async (e: React.SyntheticEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setErrors(
          parseApiError(data, "Registration failed. Please try again."),
        );
        return;
      }
    ...
    <div className="relative min-h-screen bg-[#08080f] overflow-hidden flex">
      ...
      <div className="relative z-10 hidden lg:flex w-1/2 flex-col justify-between p-16">
        ...
        <p className="text-gray-700 text-xs">
          © {new Date().getFullYear()} Dribl. All rights reserved.
        </p>
      </div>
      ...
      <div className="relative z-10 flex w-full lg:w-1/2 items-center justify-center p-8">
        <div className="w-full max-w-sm rounded-2xl overflow-hidden shadow-[0_32px_80px_rgba(0,0,0,0.6)] border border-white/[0.06]">
          ...
          <div className="bg-white px-8 py-8">
            <span className="lg:hidden block text-gray-900 text-xl font-bold tracking-tight mb-6">
              Dribl
            </span>
            ...
            {errors.length > 0 && (
              <div
                key={errors.join()}
                className="shake mb-5 flex items-start gap-3 px-4 py-3.5 rounded-xl bg-red-50 border-2 border-red-300 text-red-700 text-xs shadow-sm shadow-red-100"
              >
                <AlertCircle
                  size={15}
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <button
                type="submit"
                disabled={loading || passwordMismatch || !allRulesPassed}
