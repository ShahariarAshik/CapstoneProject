...
import type { Report, ReportItem, GetReportsResponse, BadgeColor } from "@/lib/types";

const REQUIRED_FIELDS: (keyof ReportItem)[] = [
  "id", "name", "type", "created_at", "tone",
];
...
export default function ReportsPage() {
  ...
  useEffect(() => {
    apiFetch(`${API_URL}/api/reports/get-reports`)
      ...
      .then((data: GetReportsResponse) => {
        const report = validateItems(data.reports, REQUIRED_FIELDS, "/api/reports/get-reports");
        ...
        setReports(
          data.reports.map((item: ReportItem, i: number) => {
            ...
            return {
              ...
              name:      item.name,
              type:      item.type,
              createdAt: isNaN(d.getTime()) ? item.created_at : d.toLocaleString(),
  ...
  const filtered = useMemo(() => {
    ...
    return reports.filter((r) => {
      switch (column) {
        case "Report Name": return r.name.toLowerCase().includes(q);
        case "Report Type": return r.type.toLowerCase().includes(q);
        case "Tone":        return r.tone.toLowerCase().includes(q);
        default:            return true;
      }
    });
  ...
  return (
    <div className="max-w-7xl">
      <div className="mb-8">
        <h1 className="text-3xl font-bold mb-1" style={{ color: "var(--text-1)" }}>Reports</h1>
        <p className="text-sm" style={{ color: "var(--text-2)" }}>View, edit, and download generated match and league reports.</p>
      </div>
      ...
      <div className="rounded-2xl overflow-hidden border" style={{ borderColor: "var(--border)" }}>
        <div className="px-5 py-3.5 border-b" style={{ background: "var(--bg-thead)", borderColor: "var(--border)" }}>
          <span className="text-xs font-medium" style={{ color: "var(--text-2)" }}>
            {loading ? "Loading…" : `${filtered.length} ${filtered.length === reports.length ? "total" : "matching"} reports`}
          </span>
        </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b" style={{ borderColor: "var(--border)" }}>
                {["#", "Report Name", "Report Type", "Created At", "Tone", "Actions"].map((h) => (
            ...
            <tbody>
              {loading
                ...
                : rows.length === 0
                ? <tr><td colSpan={6} className="px-5 py-10 text-center text-sm" style={{ color: "var(--text-3)" }}>No reports match your search.</td></tr>
                : rows.map((report) => (
                    <tr
                      key={report.id}
                    ...
                    >
                      <td className="px-5 py-4 text-xs font-mono" style={{ color: "var(--text-3)" }}>{report.id}</td>
                      <td className="px-5 py-4 font-medium" style={{ color: "var(--text-1)" }}>{report.name}</td>
                      <td className="px-5 py-4">
                        <Badge color={typeColor[report.type] ?? "blue"}>{report.type}</Badge>
                      </td>
                      <td className="px-5 py-4 text-xs font-mono whitespace-nowrap" style={{ color: "var(--text-2)" }}>{report.createdAt}</td>
                      <td className="px-5 py-4">
                        <Badge color={report.tone === "Comedy" ? "amber" : "indigo"}>{report.tone}</Badge>
                      </td>
                      <td className="px-5 py-4">
